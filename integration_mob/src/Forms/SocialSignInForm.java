 ...
 * @author cobwi
 */
public class SocialSignInForm extends Form {
    ...
    public SocialSignInForm(Form f, User givenUser) {
        //super("Sign In Form");
        ...
        Container welcome = FlowLayout.encloseCenter(
                new Label("Vous y êtes presque", "WelcomeBlue")
        );
        ...
        //INIT WITH GIVEN USER
        try {
            username.setText(givenUser.getNom() + givenUser.getPrenom());
        } catch (Exception ex) {
        }

        try {
            email.setText(givenUser.getEmail());
        } catch (Exception ex) {
        }

        try {
            nom.setText(givenUser.getNom());
        } catch (Exception ex) {
        }

        try {
            prenom.setText(givenUser.getPrenom());
        } catch (Exception ex) {
        }
        ...
        loginIcon.getAllStyles()
                .setMargin(RIGHT, 0);
        passwordIcon.getAllStyles()
                .setMargin(RIGHT, 0);
        FontImage.setMaterialIcon(loginIcon, FontImage.MATERIAL_PERSON_OUTLINE,
                3);
        FontImage.setMaterialIcon(passwordIcon, FontImage.MATERIAL_LOCK_OUTLINE,
                3);
        ...
        emailIcon.getAllStyles()
                .setMargin(RIGHT, 0);
        nameIcon.getAllStyles()
                .setMargin(RIGHT, 0);
        prnIcon.getAllStyles()
                .setMargin(RIGHT, 0);
        FontImage.setMaterialIcon(emailIcon, FontImage.MATERIAL_PERSON_OUTLINE,
                3);
        FontImage.setMaterialIcon(nameIcon, FontImage.MATERIAL_LOCK_OUTLINE,
                3);
        FontImage.setMaterialIcon(prnIcon, FontImage.MATERIAL_LOCK_OUTLINE,
                3);
    ...
    private void signIn(User givenUser) {
// perform some checks
//http://localhost/piproj/web/app_dev.php/new/user/ly/ly/ly@ly.com/ly/ly/1993-08-09
        ...
        user.setPhotoProfil(givenUser.getPhotoProfil());
        User persistedUser = em.persist(user, givenUser.getSocialid());
        ...
        if (persistedUser != null) {
            ...
            //stay here
            System.out.println("Persisted == null ");
        }
    }
